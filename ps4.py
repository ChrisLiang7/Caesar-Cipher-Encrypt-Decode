...
def build_coder(shift):
    ...
    """
    assert  -27 < shift < 27, 'shift %s is not between -27 and 27' % shift
...
def build_encoder(shift):
    ...
    """
    assert 0 <= shift and shift < 27, 'shift %s is not between 0 and 27' % shift
...
def apply_shifts(text, shifts):
    ...
    for i in range(len(shifts)):
        L_text = NEW_text[shifts[i][0]:]
        F_text = NEW_text[:(shifts[i][0])]
        NEW_text = F_text + apply_shift(L_text,shifts[i][1])
    return NEW_text
